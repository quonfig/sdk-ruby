...
class TestGet < Minitest::Test
  ...
  def test_get_can_return_a_string_list
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, 'my-string-list-key', {}, %w[a b c])
  end
  ...
  def test_list_on_left_side_test_1
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, 'left.hand.list.test', { 'user' => { 'name' => 'james', 'aka' => %w[happy sleepy] } }, 'correct')
  end

  # list on left side test (2)
  def test_list_on_left_side_test_2
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, 'left.hand.list.test', { 'user' => { 'name' => 'james', 'aka' => %w[a b] } }, 'default')
  end

  # list on left side test opposite (1)
  def test_list_on_left_side_test_opposite_1
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, 'left.hand.test.opposite', { 'user' => { 'name' => 'james', 'aka' => %w[happy sleepy] } }, 'default')
  end

  # list on left side test (3)
  def test_list_on_left_side_test_3
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, 'left.hand.test.opposite', { 'user' => { 'name' => 'james', 'aka' => %w[a b] } }, 'correct')
  end
end
