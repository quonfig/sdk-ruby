...
class TestGet < Minitest::Test
  ...
  def test_get_returns_nil_if_value_not_found
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, "my-missing-key", {}, nil)
  end
  ...
  def test_get_can_return_a_double
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, "my-double-key", {}, 9.95)
  end

  # get can return a string list
  def test_get_can_return_a_string_list
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, "my-string-list-key", {}, ["a", "b", "c"])
  end

  # can return a value provided by an environment variable
  def test_can_return_a_value_provided_by_an_environment_variable
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, "prefab.secrets.encryption.key", {}, "c87ba22d8662282abe8a0e4651327b579cb64a454ab0f4c170b45b15f049a221")
  end
  ...
  def test_can_return_a_value_provided_by_an_environment_variable_after_type_coercion
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, "provided.a.number", {}, 1234)
  end
  ...
  def test_can_decrypt_and_return_a_secret_value_with_decryption_key_in_in_env_var
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, "a.secret.config", {}, "hello.world")
  end
  ...
  def test_duration_90s
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, "test.duration.PT90S", {}, 90000)
  end

  # duration 1.5M
  def test_duration_1_5m
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, "test.duration.PT1.5M", {}, 90000)
  end

  # duration 0.5H
  def test_duration_0_5h
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, "test.duration.PT0.5H", {}, 1800000)
  end

  # duration test.duration.P1DT6H2M1.5S
  def test_duration_test_duration_p1dt6h2m1_5s
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, "test.duration.P1DT6H2M1.5S", {}, 108121500)
  end

  # json test
  def test_json_test
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, "test.json", {}, {"a" => 1, "b" => "c"})
  end
  ...
  def test_list_on_left_side_test_1
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, "left.hand.list.test", {"user" => {"name" => "james", "aka" => ["happy", "sleepy"]}}, "correct")
  end

  # list on left side test (2)
  def test_list_on_left_side_test_2
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, "left.hand.list.test", {"user" => {"name" => "james", "aka" => ["a", "b"]}}, "default")
  end

  # list on left side test opposite (1)
  def test_list_on_left_side_test_opposite_1
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, "left.hand.test.opposite", {"user" => {"name" => "james", "aka" => ["happy", "sleepy"]}}, "default")
  end

  # list on left side test (3)
  def test_list_on_left_side_test_3
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(self, resolver, "left.hand.test.opposite", {"user" => {"name" => "james", "aka" => ["a", "b"]}}, "correct")
  end
end
