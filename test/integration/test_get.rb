...
class TestGet < Minitest::Test
  ...
  def test_get_can_return_a_string_list
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(resolver, 'my-string-list-key', {}, %w[a b c])
  end

  # can return a value provided by an environment variable
  def test_can_return_a_value_provided_by_an_environment_variable
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(resolver, 'prefab.secrets.encryption.key', {},
  ...
  def test_duration_90s
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(resolver, 'test.duration.PT90S', {}, 90_000)
  end

  # duration 1.5M
  def test_duration_1_5m
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(resolver, 'test.duration.PT1.5M', {}, 90_000)
  end

  # duration 0.5H
  def test_duration_0_5h
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(resolver, 'test.duration.PT0.5H', {}, 1_800_000)
  end

  # duration test.duration.P1DT6H2M1.5S
  def test_duration_test_duration_p1dt6h2m1_5s
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(resolver, 'test.duration.P1DT6H2M1.5S', {}, 108_121_500)
  end
  ...
  def test_list_on_left_side_test_1
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(resolver, 'left.hand.list.test',
                                           { 'user' => { 'name' => 'james', 'aka' => %w[happy sleepy] } }, 'correct')
  end
  ...
  def test_list_on_left_side_test_2
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(resolver, 'left.hand.list.test',
                                           { 'user' => { 'name' => 'james', 'aka' => %w[a b] } }, 'default')
  end

  # list on left side test opposite (1)
  def test_list_on_left_side_test_opposite_1
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(resolver, 'left.hand.test.opposite',
                                           { 'user' => { 'name' => 'james', 'aka' => %w[happy sleepy] } }, 'default')
  end
  ...
  def test_list_on_left_side_test_3
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(resolver, 'left.hand.test.opposite',
                                           { 'user' => { 'name' => 'james', 'aka' => %w[a b] } }, 'correct')
  end
end
