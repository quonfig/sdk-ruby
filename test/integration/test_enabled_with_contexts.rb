...
class TestEnabledWithContexts < Minitest::Test
  ...
  def test_returns_true_from_global_context
    ...
    IntegrationTestHelpers.assert_enabled(self, resolver, 'feature-flag.in-seg.segment-and', { '' => { 'domain' => 'prefab.cloud' }, 'user' => { 'key' => 'michael' } }, true)
  end

  # returns false due to local context override
  def test_returns_false_due_to_local_context_override
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_enabled(self, resolver, 'feature-flag.in-seg.segment-and', { '' => { 'domain' => 'prefab.cloud' }, 'user' => { 'key' => 'james' } }, false)
  end

  # returns false for untouched scope context
  def test_returns_false_for_untouched_scope_context
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_enabled(self, resolver, 'feature-flag.in-seg.segment-and', { '' => { 'domain' => 'example.com' }, 'user' => { 'key' => 'nobody' } }, false)
  end
  ...
  def test_returns_false_due_to_partial_scope_context_override_of_user_key
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_enabled(self, resolver, 'feature-flag.in-seg.segment-and', { '' => { 'domain' => 'example.com' }, 'user' => { 'key' => 'michael' } }, false)
  end
  ...
  def test_returns_false_due_to_partial_scope_context_override_of_domain
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_enabled(self, resolver, 'feature-flag.in-seg.segment-and', { '' => { 'domain' => 'example.com', 'key' => 'prefab.cloud' }, 'user' => { 'key' => 'nobody' } }, false)
  end
  ...
  def test_returns_false_for_rule_with_different_case_on_context_property_name
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_enabled(self, resolver, 'mixed.case.property.name', { 'user' => { 'IsHuman' => 'verified' } }, false)
  end

  # returns true for matching case on context property name
  def test_returns_true_for_matching_case_on_context_property_name
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_enabled(self, resolver, 'mixed.case.property.name', { 'user' => { 'isHuman' => 'verified' } }, true)
  end
end
