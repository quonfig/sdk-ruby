...
class TestContextPrecedence < Minitest::Test
  ...
  def test_returns_the_correct_flag_value_using_the_global_context_2
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_enabled(resolver, 'mixed.case.property.name', { 'user' => { 'isHuman' => '?' } },
                                          false)
  end
  ...
  def test_returns_the_correct_get_value_using_the_global_context_1
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(resolver, 'basic.rule.config', { 'user' => { 'email' => 'test@prefab.cloud' } },
                                           'override')
  end
  ...
  def test_returns_the_correct_get_value_using_the_global_context_2
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_resolved(resolver, 'basic.rule.config', { 'user' => { 'email' => 'test@example.com' } },
                                           'default')
  end
