...
class TestDevOverrides < Minitest::Test
  ...
  def test_override_fires_when_quonfig_user_email_matches
    ...
    IntegrationTestHelpers.assert_enabled(resolver, 'feature-flag.dev-override',
                                          { 'quonfig-user' => { 'email' => 'bob@foo.com' } }, true)
  end
  ...
  def test_override_does_not_fire_when_attribute_absent_prod_simulation
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_enabled(resolver, 'feature-flag.dev-override',
                                          { 'user' => { 'email' => 'bob@foo.com' } }, false)
  end

  # override matches any email in IS_ONE_OF list
  def test_override_matches_any_email_in_is_one_of_list
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_enabled(resolver, 'feature-flag.dev-override.multi-email',
                                          { 'quonfig-user' => { 'email' => 'alice@foo.com' } }, true)
  end

  # override beats customer rule by priority
  def test_override_beats_customer_rule_by_priority
    resolver = IntegrationTestHelpers.build_resolver(@store)
    IntegrationTestHelpers.assert_enabled(resolver, 'feature-flag.dev-override.priority',
                                          { 'quonfig-user' => { 'email' => 'bob@foo.com' }, 'user' => { 'country' => 'DE' } }, true)
  end
end
